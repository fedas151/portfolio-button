(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper1")) {
            new swiper_core_Swiper(".swiper1", {
                ...
                lazyLoading: true,
                noSwiping: true,
                noSwipingClass: "arrow-slide",
                speed: 800,
                spaceBetween: 30,
                slidesOffsetAfter: 260,
                breakpoints: {},
                pagination: {
                    el: "#swiper-1 .swiper-custom-pagination1",
                    clickable: true,
    ...
    let cardTransitionTime = 1e3;
    $(".sales__card");
    let switching = false;
    $(".sales__card").on("click", flipCard);
    function flipCard() {
        if (switching) return false;
        switching = true;
        if (this.classList.contains("is-switched") == true) {
            $(this).removeClass("is-switched");
            $(this).addClass("unclick");
            setTimeout((function() {
                document.querySelector(".unclick").classList.remove("unclick");
            }), 1e3);
        } else if (this.classList.contains("is-switched") == false) {
    ...
    var tg = document.getElementById("telegram");
    var wa = document.getElementById("whatsapp");
    var ema = document.getElementById("email");
    var inp = document.getElementById("form-input");
    tg.onclick = function() {
        inp.placeholder = "tg";
    };
    wa.onclick = function() {
    ...
    $(document).ready((function() {
        $("#sendform").validate({
            ...
            messages: {
                ...
                policy: "Нужно ваше согласие на обработку персональных данных<br><br>"
            },
            submitHandler: function(form) {
                var myform = document.forms.sendform, formData = new FormData(myform), xhr = new XMLHttpRequest;
                xhr.open("POST", "../send.php");
                xhr.onreadystatechange = function() {
                    if (xhr.readyState == 3) $("#sendform").html('<p class="thank text-center">Идет отправка формы ...<br></p> ');
                    if (xhr.readyState == 4) {
                        if (xhr.status == 200) $("#sendform").html('<p class="thank text-center">Данные отправлены!<Br> Вы получите ответ в ближайшее время<Br></p> ');
                        if (xhr.status == 500) {
        ...
        Array.prototype.forEach.call(inputs, (function(input) {
            ...
            input.addEventListener("change", (function(e) {
                ...
                if (fileName) label.innerHTML = '<img src="@img/" alt=""></img>'; else label.innerHTML = labelVal;
            }));
        }));
    }));
